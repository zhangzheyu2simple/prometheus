...
func TestSampleDelivery(t *testing.T) {
	...
	m := NewQueueManager(nil, nil, dir, newEWMARate(ewmaWeight, defaultShardUpdateDuration), cfg, nil, nil, c, defaultFlushDeadline, defaultShardUpdateDuration)
...
func TestSampleDeliveryOrder(t *testing.T) {
	...
	m := NewQueueManager(nil, nil, dir, newEWMARate(ewmaWeight, defaultShardUpdateDuration), config.DefaultQueueConfig, nil, nil, c, defaultFlushDeadline, defaultShardUpdateDuration)
	m.StoreSeries(series, 0)
...
func TestShutdown(t *testing.T) {
	...
	m := NewQueueManager(nil, nil, dir, newEWMARate(ewmaWeight, defaultShardUpdateDuration), config.DefaultQueueConfig, nil, nil, c, deadline, defaultShardUpdateDuration)
...
func TestReshardRaceWithStop(t *testing.T) {
	...
	go func() {
		for {
			m = NewQueueManager(nil, nil, "", newEWMARate(ewmaWeight, defaultShardUpdateDuration), config.DefaultQueueConfig, nil, nil, c, defaultFlushDeadline, defaultShardUpdateDuration)
			m.Start()
			h.Unlock()
			h.Lock()
			m.Stop()
...
func TestCalculateDesiredsShards(t *testing.T) {
	...
	for _, c := range cases {
		...
		m := NewQueueManager(nil, nil, "", newEWMARate(ewmaWeight, defaultShardUpdateDuration), config.DefaultQueueConfig, nil, nil, client, defaultFlushDeadline, defaultShardUpdateDuration)
...
func BenchmarkStartup(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		m := NewQueueManager(nil, logger, dir,
			newEWMARate(ewmaWeight, defaultShardUpdateDuration),
			config.DefaultQueueConfig, nil, nil, c, 1*time.Minute, defaultShardUpdateDuration)
